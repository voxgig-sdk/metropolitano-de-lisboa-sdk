...
//     (\`new MetropolitanoDeLisboaSDK(...)\` / \`MetropolitanoDeLisboaSDK.test(...)\` become
//     \`MetropolitanoDeLisboaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MetropolitanoDeLisboaSDK'
...
const TEST_SEED = {"entity":{"network":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MetropolitanoDeLisboaSDK(...)\` and any
// \`MetropolitanoDeLisboaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MetropolitanoDeLisboaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MetropolitanoDeLisboaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
