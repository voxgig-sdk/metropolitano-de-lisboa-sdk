...
describe('NetworkDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new MetropolitanoDeLisboaSDK({
  ...
  test('direct-load-network', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-network', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID']
