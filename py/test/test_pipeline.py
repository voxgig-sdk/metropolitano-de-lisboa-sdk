...
def _client():
    return MetropolitanoDeLisboaSDK.test(None, None)
...
def _full_spec():
    return MetropolitanoDeLisboaSpec({
        "base": "http://h",
        "prefix": "",
        "suffix": "",
        "path": "a",
...
def _resp(status, data=None, headers=None):
    ...
    return MetropolitanoDeLisboaResponse({
        "status": status,
...
class TestMakePointAndMakeSpec:
    ...
    def test_make_spec_short_circuits_a_feature_supplied_spec(self):
        client = _client()
        ctx = _ctx(client, opname="load")
        preset = MetropolitanoDeLisboaSpec({"method": "GET"})
        ctx.out["spec"] = preset
...
class TestMakeResult:
    ...
    def test_list_op_wraps_resdata_into_entity_instances(self):
        ...
        ctx.entity = entity
        ctx.spec = _full_spec()
        ctx.result = MetropolitanoDeLisboaResult({"resdata": [{"a": 1}, {"a": 2}]})
    ...
    def test_an_empty_list_yields_an_empty_resdata_array(self):
        ...
        ctx.spec = _full_spec()
        ctx.result = MetropolitanoDeLisboaResult({"resdata": []})
        result, err = client._utility.make_result(ctx)
        assert err is None
    ...
    def test_short_circuits_on_a_preset_result(self):
        client = _client()
        ctx = _ctx(client)
        preset = MetropolitanoDeLisboaResult({"ok": True})
...
class TestMakeRequest:
    ...
    def test_short_circuits_a_feature_supplied_request(self):
        client = _client()
        ctx = _ctx(client)
        preset = MetropolitanoDeLisboaResponse({"status": 201, "statusText": "OK"})
...
class TestMakeErrorAndDone:
    ...
    def test_done_returns_resdata_on_success(self):
        client = _client()
        ctx = _ctx(client)
        ctx.result = MetropolitanoDeLisboaResult({"ok": True, "resdata": 42})
        assert client._utility.done(ctx) == 42

    def test_done_raises_the_error_when_not_ok(self):
        client = _client()
        ctx = _ctx(client)
        ctx.result = MetropolitanoDeLisboaResult({"ok": False})
        with pytest.raises(MetropolitanoDeLisboaError):
            client._utility.done(ctx)

    def test_done_cleans_ctrl_explain_on_success(self):
        client = _client()
        ctx = _ctx(client, ctrl={"explain": {"result": {"err": "x"}}})
        ctx.result = MetropolitanoDeLisboaResult({"ok": True, "resdata": 7})
    ...
    def test_make_error_returns_resdata_when_throw_is_disabled(self):
        ...
        ctx.result = MetropolitanoDeLisboaResult({"ok": False, "resdata": "fallback"})
...
class TestPrepareAuth:
    ...
    def _auth_ctx(self, client, options, headers):
        ...
        ctx.spec = None if headers is None else MetropolitanoDeLisboaSpec(
            {"headers": headers})
        return ctx
...
class TestResultHelpers:
    ...
    def test_result_headers_with_no_headers_yields_an_empty_map(self):
        client = _client()
        ctx = _ctx(client)
        ctx.response = MetropolitanoDeLisboaResponse({"status": 200})
        ctx.result = MetropolitanoDeLisboaResult({})
