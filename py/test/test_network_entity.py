    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID JSON to run live")
        client = setup["client"]
...
def _network_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("METROPOLITANO_DE_LISBOA_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("METROPOLITANO_DE_LISBOA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("METROPOLITANO_DE_LISBOA_TEST_EXPLAIN") == "TRUE",
        "live": _live,
