# MetropolitanoDeLisboa SDK test runner
...
class MetropolitanoDeLisboaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MetropolitanoDeLisboaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MetropolitanoDeLisboaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MetropolitanoDeLisboaTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = MetropolitanoDeLisboaTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MetropolitanoDeLisboaTestRunner.getenv(key)
        ...
        explain = MetropolitanoDeLisboaTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MetropolitanoDeLisboaTestRunner._test_control is not None:
            return MetropolitanoDeLisboaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MetropolitanoDeLisboaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MetropolitanoDeLisboaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MetropolitanoDeLisboaTestRunner.load_test_control()
...
def load_env_local():
    MetropolitanoDeLisboaTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return MetropolitanoDeLisboaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MetropolitanoDeLisboaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MetropolitanoDeLisboaTestRunner.load_test_control()


def live_client_options():
    return MetropolitanoDeLisboaTestRunner.live_client_options()


def live_delay_ms():
    return MetropolitanoDeLisboaTestRunner.live_delay_ms()
