...
//
// NewMetropolitanoDeLisboaSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.MetropolitanoDeLisboaSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewMetropolitanoDeLisboaSDK(opts)
}
...
func fcCandidates(client *sdk.MetropolitanoDeLisboaSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.MetropolitanoDeLisboaSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.MetropolitanoDeLisboaSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.MetropolitanoDeLisboaError
