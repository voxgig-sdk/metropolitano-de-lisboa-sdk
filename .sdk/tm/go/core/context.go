...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*MetropolitanoDeLisboaSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *MetropolitanoDeLisboaError {
	return NewMetropolitanoDeLisboaError(code, msg, ctx)
}
