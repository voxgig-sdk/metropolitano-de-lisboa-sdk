...
// MetropolitanoDeLisboa SDK retry feature
...
class MetropolitanoDeLisboaRetryFeature extends MetropolitanoDeLisboaBaseFeature
{
    ...
    public function init(MetropolitanoDeLisboaContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MetropolitanoDeLisboaContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MetropolitanoDeLisboaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MetropolitanoDeLisboaContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
