...
//    only - a typed MetropolitanoDeLisboaContext there would make every ctx
...
final class MetropolitanoDeLisboaOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MetropolitanoDeLisboaContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MetropolitanoDeLisboaSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MetropolitanoDeLisboaResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MetropolitanoDeLisboaError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MetropolitanoDeLisboaResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MetropolitanoDeLisboaError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MetropolitanoDeLisboaError(is_string($code) ? $code : '', $msg);
    }
