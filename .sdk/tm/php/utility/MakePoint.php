...
// MetropolitanoDeLisboa SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class MetropolitanoDeLisboaMakePoint
{
    public static function call(MetropolitanoDeLisboaContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof MetropolitanoDeLisboaError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = MetropolitanoDeLisboaHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = MetropolitanoDeLisboaHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
