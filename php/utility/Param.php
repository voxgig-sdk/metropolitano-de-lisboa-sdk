...
// MetropolitanoDeLisboa SDK utility: param

require_once __DIR__ . '/../core/Helpers.php';

class MetropolitanoDeLisboaParam
{
    public static function call(MetropolitanoDeLisboaContext $ctx, mixed $paramdef): mixed
    {
        $point = $ctx->point;
        $spec = $ctx->spec;
        $match_val = $ctx->match;
        ...
        $akey = '';
        if ($point) {
            $alias_map = MetropolitanoDeLisboaHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'alias'));
            if ($alias_map) {
