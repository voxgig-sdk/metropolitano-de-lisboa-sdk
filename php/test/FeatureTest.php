...
// MetropolitanoDeLisboa SDK feature test
//
...
class FtCtrl extends MetropolitanoDeLisboaControl
{
...
class FtClient
{
    ...
    private MetropolitanoDeLisboaUtility $utility;

    public function __construct(MetropolitanoDeLisboaUtility $utility, array $options)
    {
    ...
    public function get_utility(): MetropolitanoDeLisboaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MetropolitanoDeLisboaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MetropolitanoDeLisboaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MetropolitanoDeLisboaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MetropolitanoDeLisboaConfig::make_feature($name);
    ...
    private static function buildUrl(MetropolitanoDeLisboaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MetropolitanoDeLisboaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MetropolitanoDeLisboaResponse($fetched);
        }
    ...
    private function fail(MetropolitanoDeLisboaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MetropolitanoDeLisboaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
