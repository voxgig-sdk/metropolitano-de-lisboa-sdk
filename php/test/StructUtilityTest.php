...
// MetropolitanoDeLisboa SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MetropolitanoDeLisboaOmni::makeStructRunner(
    ...
    {
        return MetropolitanoDeLisboaOmni::objmodel(MetropolitanoDeLisboaOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return MetropolitanoDeLisboaOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return MetropolitanoDeLisboaOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MetropolitanoDeLisboaOmni::structfix($now, Struct::undef()))
                        !== json_encode(MetropolitanoDeLisboaOmni::structfix($val, Struct::undef()))) {
