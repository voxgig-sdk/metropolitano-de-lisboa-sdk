...
class NetworkEntityTest extends TestCase
{
    ...
    {
        $testsdk = MetropolitanoDeLisboaSDK::test(null, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $network_ref01_data = null;
        if (count($network_ref01_data_raw) > 0) {
...
function network_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["METROPOLITANO_DE_LISBOA_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new MetropolitanoDeLisboaSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["METROPOLITANO_DE_LISBOA_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
