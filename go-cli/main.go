// Command MetropolitanoDeLisboa-cli is a CLI + REPL for the MetropolitanoDeLisboa SDK,
...
const prompt = "metropolitano-de-lisboa"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "network"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      METROPOLITANO_DE_LISBOA_APIKEY (api key), METROPOLITANO_DE_LISBOA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: METROPOLITANO_DE_LISBOA_APIKEY carries the API key and
	// METROPOLITANO_DE_LISBOA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("METROPOLITANO_DE_LISBOA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("METROPOLITANO_DE_LISBOA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMetropolitanoDeLisboaSDK(opts)
