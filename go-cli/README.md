# metropolitano-de-lisboa-cli

boru-driven command-line client **and** interactive REPL for the MetropolitanoDeLisboa
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/metropolitano-de-lisboa-cli)
make build

# 2. See usage (words, entities, env vars)
./metropolitano-de-lisboa-cli --help

# 3. Provide credentials once, via the environment
export METROPOLITANO_DE_LISBOA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./metropolitano-de-lisboa-cli load 1 network            # {id:1} shorthand
./metropolitano-de-lisboa-cli load '{id:1}' network       # explicit match map

# 5. Override the API base URL for a single call
METROPOLITANO_DE_LISBOA_BASE=https://api.example.com ./metropolitano-de-lisboa-cli load 1 network

# 6. No arguments -> interactive REPL
./metropolitano-de-lisboa-cli
metropolitano-de-lisboa> load 1 network
metropolitano-de-lisboa> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/metropolitano-de-lisboa-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/metropolitano-de-lisboa-cli load 1 network
   ```
...
```sh
./metropolitano-de-lisboa-cli load 1 network          # scalar shorthand for {id:1}
./metropolitano-de-lisboa-cli load '{id:1}' network     # explicit match map
```
...
./metropolitano-de-lisboa-cli load 1 network
```
...
Run with no arguments to open a REPL (prompt `metropolitano-de-lisboa>`). Each line is
evaluated as its own boru expression:

```text
$ ./metropolitano-de-lisboa-cli
metropolitano-de-lisboa> load 1 network
metropolitano-de-lisboa> /help
metropolitano-de-lisboa> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `network`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/metropolitano-de-lisboa-cli`. |
...
(`./metropolitano-de-lisboa-cli <expr>`) and interactively (the REPL), and expressions compose the
