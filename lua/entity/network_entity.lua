...
function NetworkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NetworkEntity)
  self._name = "network"
  self._client = client
...
function NetworkEntity:get_name()
  return self._name
end


function NetworkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NetworkEntity.new(self._client, opts)
end
...
function NetworkEntity:mark_deleted()
  self._deleted = true
end


function NetworkEntity:deleted()
  return true == self._deleted
end


function NetworkEntity:data_set(args)
  if args ~= nil then
...
function NetworkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NetworkEntity:match_set(args)
  if args ~= nil then
...
function NetworkEntity:match_get()
...
function NetworkEntity:stream(action, args, callopts)
...
---@param reqmatch NetworkLoadMatch
---@param ctrl? table
---@return Network
---@return string? err
function NetworkEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NetworkEntity:_run_op(ctx, post_done)
  local utility = self._utility
