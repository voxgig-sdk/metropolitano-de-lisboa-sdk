# metropolitano-de-lisboa-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MetropolitanoDeLisboa SDK as
two agent tools — `metropolitano-de-lisboa_list` and `metropolitano-de-lisboa_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/metropolitano-de-lisboa-mcp)
make build

# 2. Provide credentials via the environment
export METROPOLITANO_DE_LISBOA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user metropolitano-de-lisboa \
  -- /absolute/path/to/metropolitano-de-lisboa-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./metropolitano-de-lisboa-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// metropolitano-de-lisboa_list: first page of records
{ "entity": "network" }
{ "entity": "network", "query": { } }

// metropolitano-de-lisboa_load: one record by id
{ "entity": "network", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/metropolitano-de-lisboa-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user metropolitano-de-lisboa \
     -- "$PWD"/dist/*/metropolitano-de-lisboa-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `metropolitano-de-lisboa_list` and `metropolitano-de-lisboa_load` tools now appear
   in new sessions. Ask the agent to *"list network using metropolitano-de-lisboa"*
   and it calls `metropolitano-de-lisboa_list` with `{"entity":"network"}`.
...
```sh
export METROPOLITANO_DE_LISBOA_APIKEY=sk_live_xxx            # API key
export METROPOLITANO_DE_LISBOA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `metropolitano-de-lisboa_list` tool
...
### Call the `metropolitano-de-lisboa_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `metropolitano-de-lisboa_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `metropolitano-de-lisboa_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `METROPOLITANO_DE_LISBOA_APIKEY` | API key sent with every request. |
| `METROPOLITANO_DE_LISBOA_BASE` | Optional override of the API base URL. |
...
```sh
./metropolitano-de-lisboa-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"metropolitano-de-lisboa_load","arguments":{"entity":"network","query":{"id":1}}}}'
```
