...
module MetropolitanoDeLisboaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MetropolitanoDeLisboaResponse.new({})
    ...
    if fetch_err
      ...
      response = MetropolitanoDeLisboaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MetropolitanoDeLisboaResponse.new(fetched)
    else
