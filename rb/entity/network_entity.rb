...
class NetworkEntity
  ...
  def make
    opts = @_entopts.dup
    NetworkEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MetropolitanoDeLisboaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MetropolitanoDeLisboaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Network.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MetropolitanoDeLisboaHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
