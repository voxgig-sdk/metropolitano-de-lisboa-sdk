...
require_relative 'MetropolitanoDeLisboa_types'
...
class MetropolitanoDeLisboaSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MetropolitanoDeLisboaHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MetropolitanoDeLisboaHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MetropolitanoDeLisboaFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MetropolitanoDeLisboaUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MetropolitanoDeLisboaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MetropolitanoDeLisboaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MetropolitanoDeLisboaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MetropolitanoDeLisboaError.new(
        "#{op}_allow",
        "MetropolitanoDeLisboaSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MetropolitanoDeLisboaError => err
    ...
    if fetched.is_a?(Hash)
      status = MetropolitanoDeLisboaHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MetropolitanoDeLisboaError.new(
        "graphql_error", "MetropolitanoDeLisboaSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MetropolitanoDeLisboaSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
