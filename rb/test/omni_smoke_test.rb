...
class OmniSmokeTest < Minitest::Test
  ...
  def smoke_pack
    runner = MetropolitanoDeLisboaOmni.make_runner(SMOKE_SPEC, MetropolitanoDeLisboaSDK.test(nil, nil))
    runner.call("smoke")
  end
  ...
  def test_runset_fails_a_wrong_result_with_omnierror
    pack = smoke_pack
    caught = assert_raises(MetropolitanoDeLisboaOmni::OmniError) do
