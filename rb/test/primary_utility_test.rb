...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = MetropolitanoDeLisboaOmni.make_runner(TEST_JSON_FILE, MetropolitanoDeLisboaSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    MetropolitanoDeLisboaError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : MetropolitanoDeLisboaOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = MetropolitanoDeLisboaOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
