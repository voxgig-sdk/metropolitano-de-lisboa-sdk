...
func TestNetworkEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		networkRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.network", setup.data)))
		var networkRef01Data map[string]any
		if len(networkRef01DataRaw) > 0 {
			networkRef01Data = core.ToMapAny(networkRef01DataRaw[0][1])
		}
		...
		if networkRef01DataDt0Loaded == nil {
...
func networkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["METROPOLITANO_DE_LISBOA_TEST_NETWORK_ENTID"])
	...
	if env["METROPOLITANO_DE_LISBOA_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewMetropolitanoDeLisboaSDK(core.ToMapAny(mergedOpts))
	}
