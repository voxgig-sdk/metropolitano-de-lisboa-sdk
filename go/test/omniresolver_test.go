...
import (
	...
	omni "github.com/voxgig-sdk/metropolitano-de-lisboa-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MetropolitanoDeLisboaError); is {
				out := map[string]any{
					"name":    "MetropolitanoDeLisboaError",
...
func omniCtx(arg any, client *sdk.MetropolitanoDeLisboaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MetropolitanoDeLisboaSDK); is {
			client = live
