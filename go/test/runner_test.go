...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("METROPOLITANO_DE_LISBOA_TEST_LIVE") == "TRUE" ||
		os.Getenv("METROPOLITANO_DE_LISBOA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("METROPOLITANO_DE_LISBOA_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.MetropolitanoDeLisboaSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MetropolitanoDeLisboaError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.MetropolitanoDeLisboaSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.MetropolitanoDeLisboaError{Msg: msg, Code: code}
}
